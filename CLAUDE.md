...
- **Competitors check #36:** Uses Places API (New) Nearby Search with the business's `location.latitude/longitude` + `primaryType` (radius 1.5 km, max 6 results, deduped on normalized name). Returns `notMeasured` if no GBP match. The synthesis prompt is given the verified list and instructed to NEVER invent competitor names — when the list is empty it falls back to industry-generic insights. Field mask includes `places.websiteUri` and `places.types` (same Enterprise SKU as rating — no extra cost); in paid the websites are scanned, see "Konkurrentjämförelse" below.
  - **`includedPrimaryTypes` 400-fix (sep 2026, roranalys.se-buggen):** Google's Nearby Search rejects some legitimate Place types as an `includedPrimaryTypes` filter value with HTTP 400 `"Unsupported types: X."` — verified live for `general_contractor`, even though Place Details happily returns it as the business's own `primaryType`. `findNearbyCompetitors()` (`places.ts`) used to let the `!res.ok` branch return `[]` silently, so the check fell to `notMeasured` with a misleading "GBP/positionsdata saknas" finding despite a complete profile. Fix: on a 400 whose body matches `/Unsupported types/i`, retry the identical search WITHOUT `includedPrimaryTypes`, then post-filter the results ourselves against the returned per-place `primaryType`/`types` (needs `places.types` in the field mask); if that post-filter leaves nothing, fall back to the unfiltered nearby list (still excluding the business's own place_id) rather than reporting notMeasured. A genuine non-400 error, or a 400 without "Unsupported types", still returns `[]` with no retry. `checkBuilder.ts`'s `buildCompetitorsNotMeasuredFinding()` now also distinguishes "sökningen misslyckades" (place has `location` + `primaryType` but the list is still empty) from "GBP/positionsdata saknas" (no place data at all), instead of always blaming a missing profile. Tests: `tests/places.test.ts` (`findNearbyCompetitors` retry/post-filter/fallback cases), `tests/checkBuilder.test.ts` (`buildCompetitorsNotMeasuredFinding`).
