...
- **Spara (free):** efter lyckad free-scan sparar `storeFreeScanInCache()` (route.ts) `{ v: 2, scanDate, scores, statuses, context }` i SQLite-tabellen `scan_cache` (`data/checkouts.db`; `cache_key TEXT PRIMARY KEY, result_json TEXT, created_at INTEGER`, index på `created_at`). `context` = `CachedScanContext` — **bara icke-Places-data** (se "Google Places-villkoren" nedan): skrapad data (`scrapedData`, `enhancedData`), Flash-rådata (teknik/FAQ/E-A-T), katalogkontroll (Tavily), AI-test, PSI, stad + `placeId`/`domainMatch`/`placeWarning`/`multipleLocationsCount`. `toCachedContext()` är en vitlista. Inget ScanResult sparas längre. `purgeExpiredScanCache()` raderar rader äldre än `SCAN_CACHE_TTL_MS` (24 h) vid varje läsning och skrivning av cachen.
...
- `meta` — domain, companyName, city, scanDate, tier, `multipleLocations: { count, cities } | null` (optional — flera distinkta kontor hittades för domänen och ingen stad angavs, se "Flera kontor / ingen stad" nedan)
...
- **Marknadsföringstexter måste matcha vad scannern faktiskt gör (Marknadsföringslagen, fix sep 2026)** — `app/layout.tsx` (title/meta), `app/components/landing/**`, `app/om-oss/page.tsx` och rapportens rubriker/FAQ får ALDRIG påstå att vi mäter/testar synlighet direkt "i" Perplexity eller Googles AI-översikter — scannern anropar ingen av dem. Det enda AI-testet (`app/lib/aiMentionChecker.ts`) frågar EN OpenAI-modell (GPT-4o-mini) via OpenRouter vad den vet om företaget — inte "ChatGPT" som produkt, inte Perplexity, inte Google AI Overview. Korrekt formulering: "förberedd för AI-sök som ChatGPT, Perplexity och Googles AI-översikter" (readiness, inte en garanti om citering av alla tre) + "vi frågar en AI-modell vad den vet om er". `app/layout.tsx`s metabeskrivning påstod tidigare att scanningen visar "exakt vad som behöver fixas för att ChatGPT, Perplexity och Google AI ska hitta och citera dig" — omskriven till ovanstående. `tests/marketingClaims.test.ts` är regressionsskydd: grep:ar de namngivna filerna efter överdrivna mätningspåståenden och låser den korrekta formuleringen i `layout.tsx`.
...
- **hreflang (#9) — deterministic language-switcher override (fix sep 2026, tvakanten.se-buggen):** check #9 is normally driven entirely by the Flash technical-signals prompt (`buildTechnicalPrompt` in `route.ts`), which only ever sees the extracted `hreflangTags` list. tvakanten.se has a same-origin "🇬🇧 ENGLISH" nav link to an English page (`/home/`) but no hreflang tags at all — Flash correctly saw an empty tag list and (per its own prompt rule "notApplicable om sajten bara har ett språk") concluded the site was single-language, never having been shown the nav link. Fix: `enhancedScraper.ts`'s `detectLanguageSwitcher($, origin)` deterministically scans `a[href]` for a SAME-ORIGIN link (external same-language sites never count) whose trimmed text is an exact language name (`english`, `svenska`, `deutsch`, ...) or contains a flag emoji (two Unicode regional-indicator code points), or whose URL has a language path segment (`/en/`, `/en-gb/`, ...) or a `lang=`/`language=`/`hl=` query param — new `EnhancedData.hasLanguageSwitcher` field. `checkBuilder.ts`'s `buildHreflangCheck(hreflangTags, hasLanguageSwitcher, flashResult)` overrides to a definite `bad` ("Sajten har innehåll på flera språk men saknar hreflang-taggar.") only when `hreflangTags` is empty AND a switcher was detected; every other combination passes the Flash result through unchanged (including the case where hreflang tags already exist, or no switcher is found — e.g. sprej.nu stays `notApplicable`). Tests: `tests/enhancedScraper.test.ts` (`detectLanguageSwitcher`, 7 cases incl. the external-site exclusion), `tests/checkBuilder.test.ts` (`buildHreflangCheck`, 3 cases).
  Path segments count only for languages OTHER than Swedish (`/en/`, `/de/`, …) — `/sv/` is deliberately excluded, since a Swedish-only site with a `/sv/` prefix on every path is not multilingual (test in `tests/enhancedScraper.test.ts`).
- **Flera kontor / ingen stad (`findBusinessByUrl`) — no random-office attribution (fix sep 2026, Issue 3/Checklist.md juni 2026, bjurfors.se-buggen):** a national chain has ONE website but MANY local offices, each with its own Google Business Profile pointing at the same domain — a Text Search on the bare domain (no city) matched 12–15 distinct bjurfors.se offices live (Linköping, Jönköping, Stockholm×4, Uppsala, Västerås, ...). `findBusinessByUrl()` used to take `data.places[0]` — whichever office Google's ranking put first (varies by call; QA saw Kungälv/Spain, this session saw Linköping) — and attribute that random office's rating/address/opening-hours/nearby-competitors to the whole report. Fix: when no `cityHint` is given, it now checks **every** domain-matching result from the plain-domain search, dedupes by city (`extractCityFromAddress()` — also fixed a dead regex in route.ts's `cityFromPlace` fallback that never matched Swedish "NNN NN City" postal codes). More than one distinct city → returns `{ _multipleLocations: { count, cities } }` and no place at all; `collectScanData()` (route.ts) nulls `placeForAnalysis` in that case, so gbpData/competitors/openingHours fall back to their existing "no GBP" path with a specific "ange stad" finding (`checkBuilder.ts`) instead of guessing. Exactly one distinct city (duplicate listings of the same office) → attributes normally. A city IS given → searches domain+city, then the bare domain, collects every domain match from both and returns the office whose `extractCityFromAddress()` equals the given city (case-insensitive) — never just Google's first hit. No office in that city: one distinct city among the matches → that office (single-office business, user typed a neighbouring town); several → `_multipleLocations` as above (e.g. "bjurfors" + Umeå), so a chain without an office in the given city never gets a random office attributed. `ScanResult.meta.multipleLocations: { count, cities } | null` — `cities` is Places-derived content (other offices' addresses): shown live, never persisted (`PLACES_CONTENT_FIELDS`/`stripPlacesContent()` strip it to `[]`, `scan_cache`'s `CachedScanContext.multipleLocationsCount` keeps only the number). Frontend: `FreeReport`/`PremiumReport` show an amber "Flera kontor hittades — ange stad" banner; `FreeReport` also has a one-step rescan form (city input, calls `onRescan` → `analyze(url, city)` in `AppShell.tsx`) so the user doesn't retype the URL. Tests: `tests/places.test.ts`, `tests/multipleLocations.test.ts`, `tests/placesContent.test.ts`, `tests/scanCache.test.ts`. Live-verified 2026-09-25: bjurfors.se no city → flagged, all three checks `notMeasured` with "ange stad"; bjurfors.se + "Göteborg" → real Göteborg office attributed; tvakanten.se (single office) no city → unchanged.
...
- **Competitors check #36:** Uses Places API (New) Nearby Search with the business's `location.latitude/longitude` + `primaryType` (radius 1.5 km, max 6 results, deduped on normalized name). Returns `notMeasured` if no GBP match. The synthesis prompt is given the verified list and instructed to NEVER invent competitor names — when the list is empty it falls back to industry-generic insights. Field mask includes `places.websiteUri` and `places.types` (same Enterprise SKU as rating — no extra cost); in paid the websites are scanned, see "Konkurrentjämförelse" below.
  - **`includedPrimaryTypes` 400-fix (sep 2026, roranalys.se-buggen):** Google's Nearby Search rejects some legitimate Place types as an `includedPrimaryTypes` filter value with HTTP 400 `"Unsupported types: X."` — verified live for `general_contractor`, even though Place Details happily returns it as the business's own `primaryType`. Text Search's `includedType` rejects the same types too ("Invalid included_type" — verified live), so there is no type-filtered Places request that works for these businesses. `findNearbyCompetitors()` (`places.ts`) used to let the `!res.ok` branch return `[]` silently, so the check fell to `notMeasured` with a misleading "GBP/positionsdata saknas" finding despite a complete profile.
    Fix: on a 400 whose body matches `/Unsupported types/i`, run a **Text Search** (`places:searchText` — a DIFFERENT billing SKU than Nearby Search: "Text Search (New)" vs "Nearby Search (New)") for the business's own Swedish `primaryTypeDisplayName` (e.g. "Generalentreprenör" for `general_contractor` — verified live: returns businesses with that exact `primaryType` nearby), biased to the same lat/lng with `locationBias.circle` (same radius), `maxResultCount: 10`, NO `includedType`. Post-filter the results ourselves against the returned `types` field (needs `places.types` in the field mask), exclude the business's own place_id, sort by distance (haversine from lat/lng) and cap at the caller's `maxResultCount`. `getPlaceDetails()`'s field mask now includes `primaryTypeDisplayName`, threaded through `placesContent.ts`'s `competitorsForPlace()` → `findNearbyCompetitors(..., primaryTypeDisplayName)`.
    Text Search's `locationBias` only weights results — it is NOT a boundary — so results farther than `radiusMeters` (default 1500 m, the "≤1,5 km" in the finding) are filtered out after the distance calculation.
    **If the post-filter leaves nothing (or no `primaryTypeDisplayName` is available), this returns `[]` — no fallback to an unfiltered "other local businesses nearby" list.** An earlier version of this fix did fall back to the unfiltered Nearby Search list when the type-filtered post-filter was empty; that produced arbitrary, unrelated neighbouring businesses labelled as "competitors" and fed them straight into the PAID competitor comparison (scraping + comparing an irrelevant business's website is worse than reporting notMeasured) — removed same day after review, before this reached a real user. A genuine non-400 error, or a 400 without "Unsupported types", still returns `[]` with no Text Search attempt. `checkBuilder.ts`'s `buildCompetitorsNotMeasuredFinding()` distinguishes "hittade inga företag av samma typ i närheten" (place has `location` + `primaryType`, search ran, found nothing) from "GBP/positionsdata saknas" (no place data at all). Tests: `tests/places.test.ts` (`findNearbyCompetitors` — Text Search fallback, post-filter, distance sort/cap, no-fallback-on-empty), `tests/placesContent.test.ts` (`primaryTypeDisplayName` threading), `tests/checkBuilder.test.ts` (`buildCompetitorsNotMeasuredFinding`).
